...
- **`skills/`** — `tdd`, `testable-app-logic`, `wireframe-html`, `regression-sweep`, `ship-non-ui`, `ship-ui`, `browser-verify`, `instructions-readme-drift-check`, `harness-init` (the onboarding/generator skill — see below), plus a thin dispatcher skill per agent (`tech-lead-review`, `tech-lead-review-with-fix`, `tech-lead-review-architecture-enforcer`, `tech-lead-review-code-quality`, `tech-lead-review-patterns`, `tech-lead-review-tests`, `ux-builder`, `ux-reviewer`, `goal-satisfaction-reviewer`) so every agent is directly reachable as `/harness-plugin:<name>`, not just spawned internally by `ship-ui`/`ship-non-ui`.
- **`agents/`** — `tech-lead-review*` (architecture-enforcer, code-quality, patterns, tests), `ux-builder`, `ux-reviewer`, `goal-satisfaction-reviewer`. (`dev-showcase-reviewer` deliberately excluded — lista-natin-specific dev-showcase feature.) **Agents are never invoked directly by slash command or @-mention — only by a skill's `Agent()` call, and only with the `harness-plugin:` namespace prefix on `subagent_type` (a bare name errors "Agent type not found").** Each agent's matching dispatcher skill above is what makes it reachable at all.
...
- **`templates/`** — `CLAUDE.md.template`, `DESIGN.md.template`, `UX-COPY-GUIDE.md.template`, `docs/tickets/CONTEXT.md.template`, `harness.config.json.example`, `vendored-hooks.json` (used by `harness-hooks-init`, see below). Structure only — every project fills its own content in.
- **`skills/harness-hooks-init`** — vendors the hook scripts (a real, git-trackable copy, not a live reference) into a target project's own `.claude/hooks/` + `.claude/settings.json`. Use this for a project the plugin's own hook delivery can't reach — most commonly a sub-repo inside a multi-repo container (see "Multi-repo containers" below) — or any project that wants the hooks without installing the rest of the plugin. Tradeoff: a vendored copy doesn't auto-update; re-run the skill to pick up a future hook improvement.
