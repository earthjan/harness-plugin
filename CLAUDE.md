...
- **`skills/`** — the installable behaviors. Includes process skills (`tdd`, `testable-app-logic`, `wireframe-html`, `regression-sweep`, `ship-non-ui`, `ship-ui`, `browser-verify`, `instructions-readme-drift-check`), the onboarding/generator skill (`harness-init`, plus `harness-hooks-init` for hook-only vendoring), and one thin dispatcher skill per agent (`tech-lead-review*`, `ux-builder`, `ux-reviewer`, `goal-satisfaction-reviewer`) so every agent is reachable directly as `/harness-plugin:<name>`, not just spawned internally by `ship-ui`/`ship-non-ui`.
- **`agents/`** — the actual agent definitions (`tech-lead-review` + its four sub-checks: architecture-enforcer, code-quality, patterns, tests; `ux-builder`; `ux-reviewer`; `goal-satisfaction-reviewer`). **Agents are never invoked directly by slash command or @-mention — only by a skill's `Agent()` call, and always with the `harness-plugin:` namespace prefix on `subagent_type`** (a bare name errors "Agent type not found"). Each agent's matching dispatcher skill is what makes it externally reachable at all. `dev-showcase-reviewer` is deliberately excluded — lista-natin-specific.
